...
export default function AllRooms() {
  ...
  // 🔥 delete popup states (NEW – UI SAFE)
  ...
  // ✅ open delete popup (REPLACED confirm)
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/rooms/${deleteId}`);
    ...
    <div className="p-6 bg-gray-50 min-h-screen relative">
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              {["Room Name", "Type", "Price", "Description", "Image", "Actions"].map(
                (head) => (
                  <th
                    key={head}
                    className="px-6 py-3 text-left text-gray-700 font-semibold uppercase tracking-wider border-b border-gray-300"
      ...
      {/* 🧾 DELETE POPUP (UI SAME AS OTHER PAGES) */}
      {showDeleteModal && (
